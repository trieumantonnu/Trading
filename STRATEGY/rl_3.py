...
class RecurrentTradingQNetwork(nn.Module):
    """
    Simpler GRU-based dueling Q-network.
...
class DQNTrader:
    ...
    def select_action_with_margin(self, state, q_margin=0.02, flat_action=1):
        q_values = self.predict_q_values(state)
        best_action = int(np.argmax(q_values))

        if best_action != flat_action:
            if q_values[best_action] - q_values[flat_action] < q_margin:
    ...
    def save(self, model_path, feature_cols=None, lookback=None):
        torch.save({
            ...
            "lookback": lookback,
            "network_type": "gru_dueling_double_dqn_validation_margin"
        }, model_path)
...
class generateSignal(Methods):
    ...
    def generate_signals(
        ...
        holding_penalty_scale=0.000005,
        wrong_side_penalty_scale=0.05,
        reward_scale=1000.0,
        ...
        save_model=True,
        model_path="./log/dqn_gru_valid_1.pth",

        # curriculum
        period_length=6000,
        target_sharpe=0.3,
        max_episodes_per_period=50,
        reward_horizon=6,
        bars_per_year=None,
        ...
        # validation and inference filter
        validation_length=1500,
        min_validation_sharpe=0.0,
        q_margin=0.02
    ):
        """
        Generate trading signals {-1, 0, 1} using a PyTorch Double-DQN agent.

        Main changes:
            1. Train-period Sharpe alone is no longer enough.
               The agent must also pass a greedy validation slice.

            2. reward_horizon is longer by default.
               This reduces chasing noisy 5-minute moves.

            3. wrong_side_penalty_scale is lower.
               This avoids overreacting to local bad trades.

            4. Q-margin filtering is used in validation and inference.
               The agent stays flat unless long/short is clearly better than flat.
        ...
        def _to_series(x, name, index):
            if isinstance(x, pd.DataFrame):
                ...
                if len(arr) != len(index):
                    raise ValueError(
                        f"{name} length {len(arr)} does not match index length {len(index)}."
                    )
        ...
        q95 = np.quantile(rolling_arr, 0.95, axis=2, keepdims=True)
        mask_95 = rolling_arr >= q95

        q05 = np.quantile(rolling_arr, 0.05, axis=2, keepdims=True)
        mask_05 = rolling_arr <= q05

        array_es_95 = np.sum(
            np.where(mask_95, rolling_arr, 0),
            axis=2
        ) / np.sum(mask_95, axis=2)

        es_95 = pd.DataFrame(
            np.round(array_es_95, 0),
            columns=["USOIL"],
            index=y_vals.iloc[-array_es_95.shape[0]:].index
        )[self.df.close.notna()]

        array_es_05 = np.sum(
            np.where(mask_05, rolling_arr, 0),
            axis=2
        ) / np.sum(mask_05, axis=2)

        es_05 = pd.DataFrame(
            np.round(array_es_05, 0),
            columns=["USOIL"],
            index=y_vals.iloc[-array_es_05.shape[0]:].index
        )[self.df.close.notna()]
        ...
        if isinstance(self.df.columns, pd.MultiIndex):
            ...
            missing_cols = [
                ...
                if (c, symbol) not in self.df.columns
            ]
            ...
            available_cols = [
                ...
                if (c, symbol) in self.df.columns
            ]
        ...
        trade_sessions_weighted = (
            2 * self.df_session["New York"].astype(int)
            + self.df_session["London"].astype(int)
        )

        trade_sessions_weighted_df = pd.DataFrame(
            trade_sessions_weighted.reindex(index=logret.index),
            columns=["USOIL"]
        )
        ...
        # Scale features
        # ---------------------------------------------
        split_idx = self.df.index.get_loc("2026-04-01 00:00:00")
        ...
        def evaluate_greedy_slice(X_data, ret_data, start_idx, end_idx):
            """
            Greedy validation on a slice that is not used for replay updates
            in the current period.
            """
            start_idx = int(start_idx)
            end_idx = int(end_idx)

            if end_idx - start_idx <= lookback + reward_horizon + 1:
                return np.nan, 0.0, 0
            ...
            first_t = start_idx + lookback - 1

            for t in range(first_t, end_idx - reward_horizon, reward_horizon):
                state_raw = X_data[t - lookback + 1: t + 1]

                if state_raw.shape[0] != lookback:
                    continue

                pos_col = np.full((lookback, 1), prev_position, dtype=np.float32)
                state = np.concatenate([state_raw, pos_col], axis=1)

                action = agent.select_action_with_margin(
                    state,
                    q_margin=q_margin,
                    flat_action=1
                )
                ...
                future_ret = float(np.sum(ret_data[t: t + reward_horizon]))

                pnl_reward = position * future_ret
                trade_cost = transaction_cost * abs(position - prev_position)
                holding_penalty = holding_penalty_scale * abs(position)

                wrong_side_penalty = 0.0
                if pnl_reward < 0:
                    wrong_side_penalty = wrong_side_penalty_scale * abs(pnl_reward)

                spread_penalty = 0.0
                if spread_penalty_scale > 0:
                    spread_penalty = spread_penalty_scale * abs(position)
                ...
                raw_returns.append(raw_reward)
                total_reward += raw_reward * reward_scale
                prev_position = position

            val_sharpe = self.compute_sharpe_ratio(
                raw_returns,
                bars_per_year=bars_per_year / reward_horizon
            )

            return val_sharpe, total_reward, len(raw_returns)
        ...
        if load_existing_model:
            ...
            if bars_per_year is None:
                bars_per_year = 12 * 24 * 252 / reward_horizon
            ...
            if n_train <= lookback + reward_horizon + validation_length + 1:
                raise ValueError(
                    "Training set is too short for the chosen lookback, "
                    "reward_horizon, and validation_length."
                )
            ...
            for i in range(periods_to_learn):
                ...
                min_period_rows = max(10000, lookback + 2 * reward_horizon + 1)
                max_period_rows = 30000

                max_start = n_train - min_period_rows - validation_length - reward_horizon - 1

                if max_start <= 0:
                    raise ValueError(
                        "Training set is too short for the chosen period_length, "
                        "validation_length, lookback, and reward_horizon."
                    )

                p_start = random.randint(0, max_start)

                sampled_period_length = min(
                    random.randint(min_period_rows, max_period_rows),
                    n_train - p_start - validation_length - reward_horizon - 1
                )

                p_end = p_start + sampled_period_length
                val_start = p_end
                val_end = min(p_end + validation_length, n_train)
                ...
                print(
                    f"\nPeriod {period_num}/{periods_to_learn} | "
                    f"train rows {p_start}:{p_end} | "
                    f"validate rows {val_start}:{val_end}"
                )
                ...
                for episode_in_period in range(max_episodes_per_period):
                    ...
                    for t in range(first_t, p_end - reward_horizon, reward_horizon):
                        state_raw = X_train[t - lookback + 1: t + 1]

                        next_state_raw = X_train[
                            t + reward_horizon - lookback + 1:
                            t + reward_horizon + 1
                        ]

                        if state_raw.shape[0] != lookback or next_state_raw.shape[0] != lookback:
                            continue
                        ...
                        next_state = np.concatenate([next_state_raw, next_pos_col], axis=1)

                        future_slice = ret_train[t: t + reward_horizon]
                        future_ret = float(np.sum(future_slice))
                        ...
                        strategy_returns.append(raw_reward)

                    if (episode_in_period + 1) % 10 == 0:
                    ...
                    period_sharpe = self.compute_sharpe_ratio(
                        strategy_returns,
                        bars_per_year=bars_per_year / reward_horizon
                    )

                    target_sharpe_period = target_sharpe * np.sqrt(
                        (max_period_rows/288) / max((p_end - p_start) / 288, 1e-8)
                    )
                    ...
                    val_sharpe, val_reward, val_steps = evaluate_greedy_slice(
                        X_train,
                        ret_train,
                        val_start,
                        val_end
                    ...
                    if (
                        np.isfinite(period_sharpe)
                        and np.isfinite(val_sharpe)
                        and period_sharpe >= target_sharpe_period
                        and val_sharpe >= min_validation_sharpe
                    ):
                        good_episode_count += 1
                        print(
                            f"Validation | "
                            f"val_reward={val_reward:.6f} | "
                            f"val_sharpe={val_sharpe:.4f} | "
                            f"val_steps={val_steps} | "
                            f"good_count={good_episode_count}/{required_good_episodes}"
                        )
                    else:
                        good_episode_count = 0
                    ...
                    # if (
                    #     episode_in_period == 0
                    #     or (episode_in_period + 1) % 5 == 0
                    #     or good_episode_count > 0
                    # ):
                    #     print(
                    #         f"Validation | "
                    #         f"val_reward={val_reward:.6f} | "
                    ...
                    if good_episode_count >= required_good_episodes:
                        print(
                            f"Period {period_num} passed validation for "
                            f"{required_good_episodes} consecutive episodes. "
                            f"Train Sharpe={period_sharpe:.4f}, "
                            f"target={target_sharpe_period:.4f}, "
                            f"validation Sharpe={val_sharpe:.4f}, "
                            f"validation target={min_validation_sharpe:.4f}. "
                            f"Move to next period."
                        )
                ...
                if not period_reached_target:
                    print(
                        f"Period {period_num} did not pass validation after "
                        f"{max_episodes_per_period} episodes. "
            ...
            print(
                f"RL training time: {train_seconds:.2f} seconds "
        ...
        # ---------------------------------------------
        # Greedy inference with Q-margin filter
        # ---------------------------------------------
        signals_arr = []
        signal_index = []
        ...
        for t in range(lookback - 1, len(X_test)):
            state_raw = X_test[t - lookback + 1: t + 1]
