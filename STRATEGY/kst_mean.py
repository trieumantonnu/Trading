...
class generateSignal(Methods):
    ...
    def generate_signals(self):
        ...
        std_ = self.df.close.ewm(span=14, adjust=False).std()
        mean_dist = ((self.df.close - self.df.close.ewm(span=14, adjust=False).mean())/std_)
        mean_cond = ((mean_dist.abs() < 0.25) & (std_ < 0.5))


        # Assumptions to be tested
        entry_cond = (kst > kst_signal * 1.1) & (std_ < 0.5) & \
                    (self.df.spread <= self.df.spread.rolling(24).quantile(0.8)) 
        exit_cond = (kst < kst_signal / 1.1)
        pre_long_signals = self.process_signal(entry_cond=entry_cond,
                                               exit_cond=exit_cond)
        
        entry_cond = (kst < kst_signal / 1.1) & \
                    (self.df.spread <= self.df.spread.rolling(24).quantile(0.8)) 
        exit_cond = (kst > kst_signal * 1.1) 
