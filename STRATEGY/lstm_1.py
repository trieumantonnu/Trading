...
class generateSignal(Methods):
    ...
    def generate_signals(
        ...
        split_idx = self.df.index.get_loc('2025-10-29 11:15:00')
        ...
        save_model=True
        model_path="./log/lstm_model2.pth"
        ...
        spread_filter = (self.df["spread"] <= self.df['spread'].ffill().rolling(24).quantile(0.8)).astype(float).reindex(pred_signal.index).fillna(False)
...
if __name__ == "__main__":
    ...
    # self.deposits input to be a df that make changes on deposits/withdrawals on some specific times
    sim.get_outputs()