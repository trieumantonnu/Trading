...
class generateSignal(Methods):
    ...
    def generate_signals(
        ...
        feat_df["spread_ok"] = (self.df["spread"] <= self.df['spread'].ffill().rolling(24).quantile(0.8)).astype(float)
        ...
        feature_df_scaled = (feature_df/feature_df.cummax()).fillna(0)
        feature_values_scaled = feature_df_scaled.values.astype(np.float32)
        ...
        save_model=True
        model_path="./log/lstm_model6.pth"
        ...
        # spread_filter = (self.df["spread"] <= self.df['spread'].ffill().rolling(24).quantile(0.8)).reindex(pred_signal.index).fillna(False)
        ...
        switch_buffer = 0.15
        exit_buffer = 0.25
        force_flip_through_flat = True
        min_hold_bars = 2

        # This assumes trainer.predict_proba(X_test) returns shape (n_samples, 3)
        # with columns ordered as [short, flat, long]
        pred_proba = trainer.predict_proba(X_test)

        proba_df = pd.DataFrame(
            pred_proba,
            index=idx_seq[split_idx:],
            columns=["p_short", "p_flat", "p_long"]
        )

        spread_filter = (self.df["spread"] <= self.df['spread'].ffill().rolling(24).quantile(0.8)).reindex(proba_df.index).fillna(False)
        
        trade_filter = (
            (self.df_session['New York'] | self.df_session['London'])
            .reindex(proba_df.index)
            .fillna(False)
        )

        final_signal = pd.Series(0, index=proba_df.index, dtype=int)
        prev_pos = 0
        hold_bars = 0

        for t in proba_df.index:
            p_short = float(proba_df.loc[t, "p_short"])
            p_flat  = float(proba_df.loc[t, "p_flat"])
            p_long  = float(proba_df.loc[t, "p_long"])

            # default desired action from probabilities and previous position
            if prev_pos == 0:
                if p_long >= enter_th and p_long > p_short + switch_buffer:
                    desired = 1
                elif p_short >= enter_th and p_short > p_long + switch_buffer:
                    desired = -1
                else:
                    desired = 0

            elif prev_pos == 1:
                if p_short > p_long + switch_buffer and p_short > p_flat + switch_buffer:
                    desired = 0 if force_flip_through_flat else -1
                elif p_flat > p_long + exit_buffer:
                    desired = 0
                else:
                    desired = 1

            else:  # prev_pos == -1
                if p_long > p_short + switch_buffer and p_long > p_flat + switch_buffer:
                    desired = 0 if force_flip_through_flat else 1
                elif p_flat > p_short + exit_buffer:
                    desired = 0
                else:
                    desired = -1

            # # optional session filter
            # if not bool(trade_filter.loc[t]):
            #     desired = 0

            # bad spread: do not open or reverse; keep current position if already in one
            if not bool(spread_filter.loc[t]):
                desired = prev_pos if prev_pos != 0 else 0

            # minimum holding period before exit or reversal
            if prev_pos != 0 and hold_bars < min_hold_bars and desired != prev_pos:
                desired = prev_pos

            final_signal.loc[t] = desired

            if desired == prev_pos and desired != 0:
                hold_bars += 1
            elif desired != 0:
                hold_bars = 1
            else:
        ...
        pred_signal = final_signal.to_frame("USOIL")
