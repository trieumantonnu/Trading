...
class RecurrentTradingQNetwork(nn.Module):
    ...
    This version removes the attention layer and the second GRU layer.
    For noisy financial rewards, a smaller network usually trains more reliably first.
    """
...
class DQNTrader:
    ...
    def save(self, model_path, feature_cols=None, lookback=None):
        torch.save({
            ...
            "lookback": lookback,
            "network_type": "gru_dueling_double_dqn"
        }, model_path)
    ...
    def generate_signals(
        ...
        if load_existing_model:
            ...
            for i in range(periods_to_learn):
                ...
                p_end = p_start + np.minimum(
                    random.randint(min_period_rows, 30000),
                    n_train - p_start
                )
                ...
                print(
                    ...
                    f"learn {p_end - p_start} rows {p_start}:{p_end}"
                )
                ...
                for episode_in_period in range(max_episodes_per_period):
                    ...
                    target_sharpe_period = target_sharpe * np.sqrt(
                        100 / max((p_end - p_start) / 288, 1e-8)
                    )
                    ...
                    if good_episode_count >= required_good_episodes:
                        print(
                            f"Period {period_num} reached target Sharpe for "
                            f"{required_good_episodes} consecutive episodes. "
                            f"Last Sharpe={period_sharpe:.4f}, "
